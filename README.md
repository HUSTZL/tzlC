...
**function declaration** Solution:编译后链接扫描两遍

**外部变量的问题，如何在静态区中分配存储单元** Solution:这个就是对函数之外的变量，偏移量相对于gp,在静态区分配存储单元，对符号表中的变量访问是，如果层号为0，那就是是由gp和偏移量获取变量地址，不是$sp和偏移量获取变量地址 *准备放弃*
